...
export class FlagLoader {
  ...
  private loadFlags(): FeatureFlags {
    ...
    return {
      ...
      LOG_LEVEL: this.getStringFlag('FF_LOG_LEVEL', 'info') as 'debug' | 'info' | 'warn' | 'error',
  ...
  private getNumberFlag(envVar: string, defaultValue: number): number {
    const value = process.env[envVar];
    if (value === undefined) return defaultValue;
  ...
  private getStringFlag(envVar: string, defaultValue: string): string {
  ...
  public isEnabled<K extends keyof FeatureFlags>(
    key: K
    ...
    return this.flags[key] as unknown as FeatureFlags[K] extends boolean ? boolean : never;
  }

  // Get flag value with type safety
  public getValue<K extends keyof FeatureFlags>(key: K): FeatureFlags[K] {
    return this.flags[key];
  }
  ...
  public validate(): { valid: boolean; errors?: string[] } {
    ...
    const booleanFlags: (keyof FeatureFlags)[] = [
      'NEW_DASHBOARD',
    ...
    const numberFlags: (keyof FeatureFlags)[] = [
    ...
    const stringFlags: (keyof FeatureFlags)[] = ['LOG_LEVEL'];
    ...
    const arrayFlags: (keyof FeatureFlags)[] = ['CORS_ORIGINS'];
...
let globalFlagLoader: FlagLoader | null = null;

// Initialize global flag loader
export function initializeFlags(environment: Environment = 'prod'): FlagLoader {
  globalFlagLoader = new FlagLoader(environment);
  return globalFlagLoader;
}

// Get global flag loader
export function getFlags(): FlagLoader {
  if (!globalFlagLoader) {
...
export function getFlagValue<K extends keyof FeatureFlags>(key: K): FeatureFlags[K] {
...
export function loadFlagsForEnvironment(environment: Environment): FeatureFlags {
  const loader = new FlagLoader(environment);
  return loader.getFlags();
}

// Flag schema validation
export function validateFlagSchema(flags: Partial<FeatureFlags>): { valid: boolean; errors?: string[] } {
  const errors: string[] = [];
