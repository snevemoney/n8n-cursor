...
| Cinematic / premium motion page | `cinematic-recipe` | 3 refs (Awwwards / godly / 21st.dev count) + named why + video hero + CTA-before-scroll roast; **premium without a named ref is a fail**; D1–D7; fail-the-build; click live (scroll + mobile). $ UNVERIFIED |
...
| Award-ref Seedance scroll-scrub site | `seedance-site` | Named award/godly refs → one continuous ≤30s 720p plate → split frames → page scrubs → scroll lock (fly-through not required) → CTA roast → D7 scroll+mobile. Cursor+Grok. Claude/Jarvis/Fable/Scrollcraft operate-never. Deploy HITL |
| orb-scroll-demo / video-to-website feels more premium than cinematic | `seedance-site` | House taste lock: plate+scrub wins. `cinematic-recipe` is the look-good cousin. Note `TASTE-WINNER-ORB-SCROLL.md`. Do not rebuild cinematic-ai-partner to add orbs. |
...
| Hunt slide/site beats / screenshot presentations / pair captions | `goal-directed-watch` | Steal first → caption timestamps → ffmpeg stills (or living-tab seek) → `CAPTIONS.md` + `watch.json`. Not a fixed-fps dump. Not video-to-website. Cap ≤24 / one id. |
...
| University / Saylor course / mentor me with those skills | `saylor-course-skill` | Speak-sheet + 1–3 COURSE-SKILLs. Consultant mentors. LANE first (`hive-os` \| `agency`). No exam dump. No invented KPI. |
| Mentor me while I build / what do I put in the system | `saylor-mentor-pass` | **Whole sitting.** One teaching beat (SAYS + NOW) then do the work. `--live` this turn. End emit the card. He does not have to ask. |
| New Saylor course closed / fold a COURSE-SKILL | `saylor-course-skill` | `saylor-fold.py --course --slug --when --never --plain`. Do not invent when/never. Then `skills-sync.py --write`. |
| Am I set up / world thinks I am a master | `saylor-mentor-pass` | Live coach is the setup. Score is a check, not the mentor. Master = one visitor-visible proof (`inbound-from-demonstrated-build`). |
