...
| **16 steal/caption leftovers** | Ledger `steal_gap` **16**. 1803 is a count, not a to-do. | Prefer steal_gap + packet + `full.txt` + ACTION TRACE **on this checkout**. Else TRIAGE. | **OBSERVED 2026-09-07:** unused pick named `2J3uX8iRNng` (`same-prompt-bench` · TRAIN). On-disk `main` was still PASS `X80ljdCPM_U`. | Do not flip `COVERAGE_LEDGER.json` |
...
**Last OBSERVED (2026-09-07 · Hive daily TRAIN Stage 1):** Researcher-shaped unused pick `2J3uX8iRNng` (`same-prompt-bench` · TRAIN). `full.txt` 6825 words + LEARNED ACTION TRACE OBSERVED. Retrieve `eval-harness` → 3 local refs. Did not write hold-outs. Did not attempt. Did not GRADE. Next = merge this pick, then Watchdog hold-outs (prefer PR 89). Yellow orphans = 8, continue.
