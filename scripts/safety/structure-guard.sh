...
# Structure Guard Script for n8n-cursor
# Enforces repository structure and safety rules
...
if [[ -f "$SCHEMA_FILE" ]]; then
  ...
  log_warn "No schema file found, using hardcoded rules"
fi
...
if [[ $forbidden_found -eq 0 ]] && [[ $forbidden_strings_found -eq 0 ]] && [[ $missing_files -eq 0 ]]; then
  log_info "PASSED: Structure Guard"
  if [[ $missing_dirs -eq 0 ]]; then
    log_info "PASSED: All required directories present"
  else
    log_warn "Warning: Some directories missing - non-critical"
  fi
  exit 0
else
  log_error "FAILED: Structure Guard"
