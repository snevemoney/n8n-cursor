...
SCENARIO_BANK: dict[str, list[str]] = {
    ...
    "HITL Operator": [
        ...
        "Approve a sensitive client email draft (draft-only; Evens sends).",
        ...
        "Approve an important-lead outreach draft (draft-only).",
    ...
    "Communications Manager": [
        ...
        "Important client email → summarize → draft → HITL (draft-only; Evens sends).",
        ...
        "New contact outreach draft → HITL (draft-only; do not send).",
        ...
        "Low-risk thread → no ack-reply; classify only; never send.",
...
SUPPRESSION_RULES: dict[str, dict[str, Any]] = {
    ...
    "Communications Manager": {
        ...
        "always_hitl": True,
        "notes": (
            "Read/classify/label/archive/summarize/extract/draft = yes; send is removed (draft-only; Evens sends); "
