# Workflow — retest-the-wall-then-hitl-callback
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** retest-the-wall-then-hitl-callback
**Tape:** `3QclAjmu5Tw` · caption-only · 2483 words read
**Compiled:** 2026-08-14
...
- **Transcript:** `packets/3QclAjmu5Tw/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `retest-the-wall-then-hitl-callback` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/3QclAjmu5Tw/full.txt`
