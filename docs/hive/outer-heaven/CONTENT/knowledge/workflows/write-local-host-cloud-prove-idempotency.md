# Workflow — write-local-host-cloud-prove-idempotency
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** write-local-host-cloud-prove-idempotency
**Tape:** `UGIZnh6HNLc` · caption-only · 5267 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-UGIZnh6HNLc-01, K-UGIZnh6HNLc-02, K-UGIZnh6HNLc-03
...
- **Transcript:** `packets/UGIZnh6HNLc/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `write-local-host-cloud-prove-idempotency` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-UGIZnh6HNLc-01, K-UGIZnh6HNLc-02
- **dissent_ids:** none — caption-only speech≠behavior stays on LEARNED
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/UGIZnh6HNLc/full.txt`
