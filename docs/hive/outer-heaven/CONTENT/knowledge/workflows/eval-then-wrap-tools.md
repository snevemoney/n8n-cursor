# Workflow — eval-then-wrap-tools
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** eval-then-wrap-tools
**Tape:** `X80ljdCPM_U` · caption-only · 4440 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-X80ljdCPM_U-01, K-X80ljdCPM_U-02, K-X80ljdCPM_U-03
...
- **Transcript:** `packets/X80ljdCPM_U/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `eval-then-wrap-tools` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-X80ljdCPM_U-01, K-X80ljdCPM_U-02
- **dissent_ids:** none — caption-only speech≠behavior stays on LEARNED
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/X80ljdCPM_U/full.txt`
