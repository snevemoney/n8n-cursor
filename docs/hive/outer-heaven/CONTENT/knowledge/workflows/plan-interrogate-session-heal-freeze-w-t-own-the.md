# Workflow — plan-interrogate-session-heal-freeze-w-t-own-the
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** plan-interrogate → session-heal → freeze-W+T → own-the-KPI
**Tape:** `vFepZE_wrfg` · caption-only · 6850 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-vFepZE_wrfg-01, K-vFepZE_wrfg-02, K-vFepZE_wrfg-03, K-vFepZE_wrfg-04
- **dissent_ids:** K-vFepZE_wrfg-05, K-vFepZE_wrfg-06, K-vFepZE_wrfg-07, K-vFepZE_wrfg-08
...
- **Transcript:** `packets/vFepZE_wrfg/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `plan-interrogate-session-heal-freeze-w-t-own-the` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-vFepZE_wrfg-01, K-vFepZE_wrfg-02
- **dissent_ids:** K-vFepZE_wrfg-05
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/vFepZE_wrfg/full.txt`
...
- **dissent kept visible:** K-vFepZE_wrfg-05, K-vFepZE_wrfg-06, K-vFepZE_wrfg-07, K-vFepZE_wrfg-08
