# Workflow — one-key-two-surfaces-not-in-chat
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** one-key-two-surfaces-not-in-chat
**Tape:** `EiHVBPyvTiE` · caption-only · 420 words read
**Compiled:** 2026-08-14
...
- **Transcript:** `packets/EiHVBPyvTiE/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `one-key-two-surfaces-not-in-chat` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/EiHVBPyvTiE/full.txt`
