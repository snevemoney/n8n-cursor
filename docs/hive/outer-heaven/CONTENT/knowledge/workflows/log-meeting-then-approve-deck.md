# Workflow — log-meeting-then-approve-deck
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** log-meeting-then-approve-deck
**Tape:** `-Q_P7HFydZk` · caption-only · 812 words read
**Compiled:** 2026-08-14
...
- **Transcript:** `packets/-Q_P7HFydZk/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `log-meeting-then-approve-deck` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K--Q_P7HFydZk-01, K--Q_P7HFydZk-02
- **dissent_ids:** none — caption-only speech≠behavior stays on LEARNED
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/-Q_P7HFydZk/full.txt`
