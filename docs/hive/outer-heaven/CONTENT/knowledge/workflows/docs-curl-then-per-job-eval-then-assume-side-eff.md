# Workflow — docs-curl-then-per-job-eval-then-assume-side-eff
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** docs-curl-then-per-job-eval-then-assume-side-effect
**Tape:** `Vb1SwBAn9cQ` · caption-only · 5836 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-Vb1SwBAn9cQ-01, K-Vb1SwBAn9cQ-02, K-Vb1SwBAn9cQ-03
...
- **Transcript:** `packets/Vb1SwBAn9cQ/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `docs-curl-then-per-job-eval-then-assume-side-eff` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-Vb1SwBAn9cQ-01, K-Vb1SwBAn9cQ-02
- **dissent_ids:** none — caption-only speech≠behavior stays on LEARNED
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/Vb1SwBAn9cQ/full.txt`
