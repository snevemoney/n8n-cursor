# Workflow — clocked-rehearsal-then-path-vs-data
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** clocked-rehearsal-then-path-vs-data
**Tape:** `bWhjRLX0jpo` · caption-only · 2318 words read
**Compiled:** 2026-08-14
...
- **Transcript:** `packets/bWhjRLX0jpo/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `clocked-rehearsal-then-path-vs-data` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/bWhjRLX0jpo/full.txt`
