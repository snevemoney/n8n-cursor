# Workflow — safe-hours-then-constraint
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** safe-hours-then-constraint
**Tape:** `eFOTQpbGcy8` · caption-only · 2689 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-eFOTQpbGcy8-01, K-eFOTQpbGcy8-02
...
- **Transcript:** `packets/eFOTQpbGcy8/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `safe-hours-then-constraint` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/eFOTQpbGcy8/full.txt`
