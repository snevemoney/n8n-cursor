# Workflow — filter-row-then-llm
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** filter-row-then-llm
**Tape:** `QCjMBOEhpLE` · caption-only · 3372 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-QCjMBOEhpLE-01, K-QCjMBOEhpLE-02, K-QCjMBOEhpLE-03, K-QCjMBOEhpLE-04
...
- **Transcript:** `packets/QCjMBOEhpLE/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `filter-row-then-llm` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-QCjMBOEhpLE-01, K-QCjMBOEhpLE-02
- **dissent_ids:** none — caption-only speech≠behavior stays on LEARNED
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/QCjMBOEhpLE/full.txt`
