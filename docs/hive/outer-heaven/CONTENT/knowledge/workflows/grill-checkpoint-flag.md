# Workflow — grill-checkpoint-flag
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** grill-checkpoint-flag
**Tape:** `c0kaKxM2pHg` · caption-only · 1854 words read
**Compiled:** 2026-08-14
...
- **Transcript:** `packets/c0kaKxM2pHg/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `grill-checkpoint-flag` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/c0kaKxM2pHg/full.txt`
