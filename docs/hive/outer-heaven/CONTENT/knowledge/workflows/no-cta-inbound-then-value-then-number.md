# Workflow — no-cta-inbound-then-value-then-number
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** no-cta-inbound-then-value-then-number
**Tape:** `8C6iCpJ9HPo` · caption-only · 2696 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-8C6iCpJ9HPo-01, K-8C6iCpJ9HPo-02
...
- **Transcript:** `packets/8C6iCpJ9HPo/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `no-cta-inbound-then-value-then-number` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/8C6iCpJ9HPo/full.txt`
