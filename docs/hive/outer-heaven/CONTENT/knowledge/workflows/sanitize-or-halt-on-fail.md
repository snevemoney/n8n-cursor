# Workflow — sanitize-or-halt-on-fail
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** sanitize-or-halt-on-fail
**Tape:** `NQhsLVmuItA` · caption-only · 339 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-NQhsLVmuItA-01, K-NQhsLVmuItA-02
...
- **Transcript:** `packets/NQhsLVmuItA/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `sanitize-or-halt-on-fail` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/NQhsLVmuItA/full.txt`
