# Workflow — lookup-then-seven-verbs
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** lookup-then-seven-verbs
**Tape:** `glM8godEcic` · caption-only · 484 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-glM8godEcic-01, K-glM8godEcic-02
...
- **Transcript:** `packets/glM8godEcic/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `lookup-then-seven-verbs` → run `deep-video-learning`
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/glM8godEcic/full.txt`
