# Workflow — curl-ticket-expand-prompt-sanitize-hitl-frame
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** curl-ticket → expand-prompt → sanitize → HITL frame
**Tape:** `Vm8QOo9MiC4` · caption-only · 6830 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-Vm8QOo9MiC4-01, K-Vm8QOo9MiC4-02, K-Vm8QOo9MiC4-03
...
- **Transcript:** `packets/Vm8QOo9MiC4/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `curl-ticket-expand-prompt-sanitize-hitl-frame` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-Vm8QOo9MiC4-01, K-Vm8QOo9MiC4-02
- **dissent_ids:** none — caption-only speech≠behavior stays on LEARNED
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/Vm8QOo9MiC4/full.txt`
