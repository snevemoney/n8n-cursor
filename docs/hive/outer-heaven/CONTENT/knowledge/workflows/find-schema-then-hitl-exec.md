# Workflow — find-schema-then-hitl-exec
Status: compiled
Protocol: workflow-compiler
**Provenance:** WORKFLOW → PATTERN → ATOMS → TRANSCRIPT
**Title:** find-schema-then-hitl-exec
**Tape:** `mPflFTQUCGk` · caption-only · 515 words read
**Compiled:** 2026-08-14
...
- **support_ids:** K-mPflFTQUCGk-01, K-mPflFTQUCGk-02, K-mPflFTQUCGk-03, K-mPflFTQUCGk-04
- **dissent_ids:** K-mPflFTQUCGk-05, K-mPflFTQUCGk-06, K-mPflFTQUCGk-07
...
- **Transcript:** `packets/mPflFTQUCGk/full.txt` @ UNKNOWN

### 2. Run named skill on Cursor+Grok
- **IF:** IF the tape machine is this `find-schema-then-hitl-exec` → run `deep-video-learning`
- **Do:** Follow the skill card. Map on-tape vendors to Cursor skills + Grok Bot + `cursor-ide-browser`.
- **Hive skill:** `deep-video-learning`
- **pattern_ids:** none — hive skill
- **support_ids:** K-mPflFTQUCGk-01, K-mPflFTQUCGk-02
- **dissent_ids:** K-mPflFTQUCGk-05
...
## Audits
- **coverage:** pass — five tasks sourced to hive skill or LEARNED/atoms/`packets/mPflFTQUCGk/full.txt`
...
- **dissent kept visible:** K-mPflFTQUCGk-05, K-mPflFTQUCGk-06, K-mPflFTQUCGk-07
