...
|-----------|-----------|---------|------|-------------|--------|
| Hive Error Heal Notify | `error-heal-notify.json` | POST `/webhook/hive-error-heal` (`onReceived`) | **yes** | Normalize failure → register + Telegram; propose PR | ACTIVE **FACT** |
...
**Shape FACT:** `Webhook` → `Normalize Error` (code) → `Register Scorpion` → `Alert Outer Heaven` (Telegram).
