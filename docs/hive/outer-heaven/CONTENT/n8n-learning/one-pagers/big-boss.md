...
**Shape FACT:** Schedule → parallel `Fetch 24h Missions` + `Fetch Golden Paths` → `Wait For Both Fetches` (mode=append) → `Build Digest` → `Alert Grok Watchdog`. n8n notify sink = Grok Watchdog webhook env GROK_WATCHDOG_WEBHOOK_URL. Telegram send deactivated (not a fallback).
