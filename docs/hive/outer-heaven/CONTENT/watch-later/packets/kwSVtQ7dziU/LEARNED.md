...
### Machine: macro-action-metric-named-workers
- **Workflow / loop:** hive remap of `macro-action-metric-named-workers` (HITL)
- **Operate-never payload:** quote Karpathy $ / AutoResearch as FACT · whole-onion one sit
- **Hive run (existing skills only):** `interview-to-desk`, `golden-test-loop`, `ask-principal`, `slice-build`, `click-live-site`

## HOW THEY BUILT
- product/tool/workflow/website: **thesis/build** (caption-only).
- steps they showed: stolen machine `macro-action-metric-named-workers` — see Stolen machines.
- tools they used (on-tape vs we already have): on-tape vendors operate-never. Ours: `interview-to-desk`, `golden-test-loop`, `ask-principal`, `slice-build`, `click-live-site`.
- what we can emit from our factory: system-upgrade `macro-action-metric-named-workers` + labeled merges. No website-assets. No duplicate mint.

## ACTION TRACE
- timestamps — UNKNOWN (caption-only; no living watch this sit)
- state transitions — see LEARNED stolen machine `macro-action-metric-named-workers`
- UNKNOWN blockers: clicks / on-screen prompts / tape $ — BLOCKING? no for hive remap

## CAPABILITY CARD
CAPABILITY: macro-action-metric-named-workers
SOURCE: `kwSVtQ7dziU` No Priors: AI, Machine Learning, Tech, & Startups (caption-only)
DEMONSTRATED OUTCOME: spoken machine; hive remap this sit
PREREQUISITES: existing hive skills listed above
WORKFLOW: hive remap of `macro-action-metric-named-workers` (HITL)
SKILL CANDIDATES: interview-to-desk, golden-test-loop, ask-principal, slice-build, click-live-site (merge, do not mint duplicate)
SYSTEM UPGRADE CANDIDATES: UPG-kwSVtQ7dziU-macro-action-metric-named-workers
