# Ingest — Nate Herk 3 Hidden Data Table Hacks for Smarter AI Agents

**Video id:** `lcNN3X9gXls`  
**URL:** https://www.youtube.com/watch?v=lcNN3X9gXls  
**Title:** 3 Hidden Data Table Hacks for Smarter AI Agents  
**Channel:** Nate Herk | AI Automation  
**Kind:** video  
**Duration:** 15:17  
**Captions:** yt-dlp `en-orig` json3 — **3791 words**. Not invented.  
**Date:** 2026-08-14  
**Uploaded:** 2025-09-26  
**ICP:** `us` only. Live hunt stays `local-pro` / Normand. No new `icp_id`.

## Load (17 desks)

```
/Users/evenslouis/n8n-cursor/docs/hive/outer-heaven/CONTENT/watch-later/packets/lcNN3X9gXls/full.txt
```

Mirror (same bytes): `docs/hive/outer-heaven/CONTENT/watch-later/packets/lcNN3X9gXls/transcripts/full.txt`  
VTT/json3: `docs/hive/outer-heaven/CONTENT/watch-later/packets/lcNN3X9gXls/transcripts/lcNN3X9gXls.en-orig.json3`

## Transcript said (pointer — read full.txt)

Caption ingest only. Read `full.txt` (3791 words). No invented summary. 17 desks not spawned.

## Steal vs operate-never

| Steal (existing machines only) | Operate-never |
|--------------------------------|---------------|
| Catalog pointer + `full.txt` for later `channel-walk` / `coverage-loop` | Spawn 17 this scrape · dump take folders · treat Nate $ as FACT |
| `ask-principal` on send / pay / deploy / book / publish | Join Skool / install his sold n8n templates |
| Operate Cursor + Grok Bot | Unpark Normand / start a Path A client |

**17 desks:** not spawned.
