...
const PAGES = ["index.html", "walkthrough.html"];
...
test("shell: every primary CTA lands on the product door, contrast card points back at it", () => {
  ...
  assert.equal(byCta["nav-door"], "walkthrough.html");
  assert.equal(byCta["hero-primary"].split("#")[0], "walkthrough.html");
  assert.equal(byCta["door-primary"], "walkthrough.html");
  assert.ok(byCta["door-contrast"].startsWith("walkthrough.html"), "contrast CTA resolves to the door");
  assert.match(html, /Spec in\./);
  assert.match(html, /class="card friction" data-muted="true"/, "friction contrast card present and struck");
});

test("door page: empty line, load spec, advance/back controls, six stations, locked ship, hold-out reveal, demo states", () => {
  const html = read("walkthrough.html");
  assert.match(html, /id="line" data-state="empty"/);
  assert.match(html, /id="line-state"[^>]*aria-live="polite"/);
  assert.match(html, /id="stations"/);
  assert.match(html, /id="advance-btn" disabled/);
  assert.match(html, /id="back-btn" disabled/);
  assert.match(html, /id="artifact"/);
  assert.match(html, /Merge ≠ ship/);
  ...
  assert.match(html, /<script type="module" src="assets\/line-ui.js">/);
});
...
test("no-agency-talk lock: pitch phrasing only inside the struck friction card", () => {
  for (const p of PAGES) {
    const html = read(p).replace(/<s>[\s\S]*?<\/s>/g, "").replace(/<[^>]*data-contrast[^>]*>[\s\S]*?<\/[a-z]+>/g, "");
    assert.doesNotMatch(html, /we do ai|i do ai|ai agency|discovery call|synergies/i, `${p} must not read as an AI agency pitch`);
  }
});

test("CSS: hidden attribute beats display:flex/grid; mobile breakpoint; reduced motion; tokens block", () => {
  const css = read("assets/outer-heaven.css");
  ...
  assert.match(css, /--brand:\s*#c9a227/, "placeholder token declared where the SSOT swap lands");
  assert.match(css, /VERTICALS\.outer\-heaven/, "SSOT pointer kept in the tokens comment");
});
