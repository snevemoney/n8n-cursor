...
test("shell: every primary CTA lands on the product door, contrast card points back at it", () => {
  ...
  assert.equal(byCta["nav-door"], "manual.html");
  assert.equal(byCta["hero-primary"].split("#")[0], "manual.html");
  assert.equal(byCta["door-primary"], "manual.html");
  assert.ok(byCta["door-contrast"].startsWith("manual.html"), "contrast CTA resolves to the door");
  assert.match(html, /Manuals you hold\./);
  assert.match(html, /class="card friction" data-muted="true"/, "friction contrast card present and struck");
});

test("door page: empty hand, shelf of manuals, hand frame, open/put-back controls, thumb-tabs, steps list, demo states", () => {
  const html = read("manual.html");
  assert.match(html, /id="desk" data-state="empty"/);
  assert.match(html, /id="desk-state"[^>]*aria-live="polite"/);
  assert.match(html, /id="shelf"/);
  assert.match(html, /id="hand"/);
  assert.match(html, /id="open-btn" disabled/);
  assert.match(html, /id="putback-btn" disabled/);
  assert.match(html, /id="tabs"[^>]*role="tablist"/);
  assert.match(html, /id="steps"/);
  assert.match(html, /Nothing in hand/);
  ...
  assert.match(html, /<script type="module" src="assets\/manual-ui.js">/);
});
...
test("CSS: hidden attribute beats display:flex/grid; mobile breakpoint; reduced motion; tokens block", () => {
  const css = read("assets/field-manuals.css");
  ...
  assert.match(css, /--brand:\s*#ff7a3d/, "placeholder token declared where the SSOT swap lands");
  assert.match(css, /VERTICALS\.field\-manuals/, "SSOT pointer kept in the tokens comment");
});
