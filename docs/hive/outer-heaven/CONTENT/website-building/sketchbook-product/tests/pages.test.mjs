...
test("shell: every primary CTA lands on the product door, contrast card points back at it", () => {
  ...
  assert.equal(byCta["nav-door"], "book.html");
  assert.equal(byCta["hero-primary"].split("#")[0], "book.html");
  assert.equal(byCta["door-primary"], "book.html");
  assert.ok(byCta["door-contrast"].startsWith("book.html"), "contrast CTA resolves to the door");
  assert.match(html, /A site that turns/);
  assert.match(html, /class="card friction" data-muted="true"/, "friction contrast card present and struck");
});

test("door page: closed cover, open control, prev/next, position pill, spread panes, keyboard hint, demo states", () => {
  const html = read("book.html");
  assert.match(html, /id="book" data-state="closed"/);
  assert.match(html, /id="book-state"[^>]*aria-live="polite"/);
  assert.match(html, /id="open-btn"/);
  assert.match(html, /id="prev-btn" disabled/);
  assert.match(html, /id="next-btn"/);
  assert.match(html, /id="spread"/);
  assert.match(html, /id="page-left"/);
  assert.match(html, /id="page-right"/);
  ...
  assert.match(html, /<script type="module" src="assets\/pages-ui.js">/);
});
...
test("CSS: hidden attribute beats display:flex/grid; mobile breakpoint; reduced motion; tokens block", () => {
  const css = read("assets/sketchbook.css");
  ...
  assert.match(css, /--brand:\s*#e0b878/, "placeholder token declared where the SSOT swap lands");
  assert.match(css, /VERTICALS\.sketchbook/, "SSOT pointer kept in the tokens comment");
});
