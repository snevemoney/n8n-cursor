...
const PAGES = ["index.html", "verify.html"];
...
test("shell: every primary CTA lands on the product door, contrast card points back at it", () => {
  ...
  assert.equal(byCta["nav-door"], "verify.html");
  assert.equal(byCta["hero-primary"].split("#")[0], "verify.html");
  assert.equal(byCta["door-primary"], "verify.html");
  assert.ok(byCta["door-contrast"].startsWith("verify.html"), "contrast CTA resolves to the door");
  assert.match(html, /Your sources\. Your voice\./);
  assert.match(html, /class="card friction" data-muted="true"/, "friction contrast card present and struck");
});

test("door page: empty state, disabled Verify Now, claim list, voice meter, gate, toast, demo states", () => {
  const html = read("verify.html");
  assert.match(html, /id="gate" data-state="empty"/);
  assert.match(html, /id="gate-state"[^>]*aria-live="polite"/);
  assert.match(html, /id="verify-btn" disabled/);
  assert.match(html, /Verify Now/);
  assert.match(html, /id="voice-meter"/);
  assert.match(html, /Words changed/);
  ...
  assert.match(html, /<label for="draft">/);
  assert.match(html, /<script type="module" src="assets\/verify-ui.js">/);
});
...
test("voice lock: no rewrite promise anywhere except the struck friction card", () => {
  for (const p of PAGES) {
    const html = read(p).replace(/<s>[\s\S]*?<\/s>/g, "").replace(/<[^>]*data-contrast[^>]*>[\s\S]*?<\/[a-z]+>/g, "");
    assert.doesNotMatch(html, /rewrite your|polished version|improve your writing/i, `${p} must not read as a rewriting tool`);
  }
});

test("CSS: hidden attribute beats display:flex/grid; mobile breakpoint; reduced motion; tokens block", () => {
  const css = read("assets/proofcheck.css");
  ...
  assert.match(css, /--brand:\s*#1fb6a6/, "placeholder token declared where the SSOT swap lands");
  assert.match(css, /VERTICALS\.proofcheck\-qc/, "SSOT pointer kept in the tokens comment");
});
