...
const PAGES = ["index.html", "shelf.html"];
...
test("shell: every primary CTA lands on the product door, contrast card points back at it", () => {
  ...
  assert.equal(byCta["nav-door"], "shelf.html");
  assert.equal(byCta["hero-primary"].split("#")[0], "shelf.html");
  assert.equal(byCta["door-primary"], "shelf.html");
  assert.ok(byCta["door-contrast"].startsWith("shelf.html"), "contrast CTA resolves to the door");
  assert.match(html, /Working volumes\./);
  assert.match(html, /class="card friction" data-muted="true"/, "friction contrast card present and struck");
});

test("door page: shelved state, spine list, empty desk, reshelve-all disabled, pill, demo states", () => {
  const html = read("shelf.html");
  assert.match(html, /id="shelf-stage" data-state="empty"/);
  assert.match(html, /id="shelf-state"[^>]*aria-live="polite"/);
  assert.match(html, /id="shelf"[^>]*role="group"/);
  assert.match(html, /id="desk"/);
  assert.match(html, /id="reshelve-all" disabled/);
  assert.match(html, /Nothing pulled/);
  ...
  assert.match(html, /<script type="module" src="assets\/shelf-ui.js">/);
});
...
test("CSS: hidden attribute beats display:flex/grid; mobile breakpoint; reduced motion; tokens block", () => {
  const css = read("assets/working-volumes.css");
  ...
  assert.match(css, /--brand:\s*#d9564f/, "placeholder token declared where the SSOT swap lands");
  assert.match(css, /VERTICALS\.working\-volumes/, "SSOT pointer kept in the tokens comment");
});
