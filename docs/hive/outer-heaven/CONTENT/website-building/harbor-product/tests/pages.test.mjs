...
const PAGES = ["index.html", "job.html"];
...
test("shell: every job CTA lands on job.html, service CTAs preselect, no tel: door, no quote figure", () => {
  const html = read("index.html");
  const byCta = ctas(html);
  assert.equal(byCta["nav-request-job"], "job.html");
  assert.equal(byCta["hero-request-job"], "job.html");
  assert.equal(byCta["services-request-job"], "job.html");
  assert.equal(byCta["service-gutter"], "job.html?service=Gutter+%26+roofline");
  assert.equal(byCta["service-deck"], "job.html?service=Deck+%26+fence+repair");
  assert.equal(byCta["service-plumbing"], "job.html?service=Small+plumbing");
  assert.equal(byCta["see-booked"], "job.html?state=booked");
  assert.match(html, /Describe the job\./);
  assert.match(html, /the visit\./);
  assert.doesNotMatch(html, /href="tel:/, "the door is not a phone number");
});

test("job page: empty frame, stepper, two steps, disabled next, hidden book, grid group, outcome, friction, demo states", () => {
  const html = read("job.html");
  assert.match(html, /id="job-frame" data-state="empty"/);
  assert.match(html, /id="job-state"[^>]*aria-live="polite"/);
  assert.match(html, /<ol class="stepper" id="stepper"/);
  assert.match(html, /id="step-job"/);
  assert.match(html, /id="step-visit" hidden/);
  assert.match(html, /id="grid" role="group"/);
  assert.match(html, /id="next-btn" disabled/);
  assert.match(html, /id="book-btn" hidden disabled/);
  ...
  for (const id of ["service", "address", "note", "homeowner"]) {
    assert.match(html, new RegExp(`<label for="${id}">`), `label for ${id}`);
  }
  assert.match(html, /<script type="module" src="assets\/job-ui.js">/);
});

test("CapEx locks: sample labels on every page, no external scripts, no payment, quote, or outbound chrome", () => {
  for (const p of PAGES) {
    const html = read(p);
    ...
    assert.doesNotMatch(visible, /\$\s?\d/, `${p} must not carry a price or quote figure`);
  }
});
...
test("CSS: tokens define the brand, shell is token-only, hidden wins, mobile + reduced motion present", () => {
  ...
  assert.match(tokens, /VERTICALS\.harbor/, "tokens name the SSOT swap point");
