...
const PAGES = ["index.html", "stage.html"];
...
test("shell: every primary CTA lands on the product door, contrast card points back at it", () => {
  ...
  assert.equal(byCta["nav-door"], "stage.html");
  assert.equal(byCta["hero-primary"].split("#")[0], "stage.html");
  assert.equal(byCta["door-primary"], "stage.html");
  assert.ok(byCta["door-contrast"].startsWith("stage.html"), "contrast CTA resolves to the door");
  assert.match(html, /Motion we own\./);
  assert.match(html, /class="card friction" data-muted="true"/, "friction contrast card present and struck");
});

test("door page: dark stage, canvas element, start/pause controls, density/style controls, readout, no stock media, demo states", () => {
  const html = read("stage.html");
  assert.match(html, /id="stage" data-state="dark"/);
  assert.match(html, /id="stage-state"[^>]*aria-live="polite"/);
  assert.match(html, /<canvas[^>]*id="globe"/);
  assert.match(html, /id="start-btn"/);
  assert.match(html, /id="pause-btn" disabled/);
  assert.match(html, /id="density"/);
  assert.match(html, /id="style"/);
  assert.match(html, /id="readout"/);
  assert.match(html, /0 stock assets/);
  assert.doesNotMatch(html, /\.(mp4|webm|mov|gif|glb|lottie)\b/i, "no stock media referenced");
  ...
  assert.match(html, /<script type="module" src="assets\/globe-ui.js">/);
});
...
test("CSS: hidden attribute beats display:flex/grid; mobile breakpoint; reduced motion; tokens block", () => {
  const css = read("assets/betawise.css");
  ...
  assert.match(css, /--brand:\s*#3fd0a0/, "placeholder token declared where the SSOT swap lands");
  assert.match(css, /VERTICALS\.betawise\-earth/, "SSOT pointer kept in the tokens comment");
});
