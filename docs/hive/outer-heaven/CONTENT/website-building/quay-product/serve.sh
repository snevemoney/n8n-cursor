#!/bin/bash
# Quay Team product preview :4844. 127.0.0.1 only. CapEx / preview. Never deploys.
...
if lsof -nP -iTCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  echo "already listening on 127.0.0.1:$PORT (quay) pid=$existing"
else
  python3 -m http.server "$PORT" --bind 127.0.0.1 --directory "$DIR" >/dev/null 2>&1 &
  echo $! > "$PIDDIR/$PORT.pid"
  echo "started 127.0.0.1:$PORT → quay pid=$!"
fi

echo
echo "Shell   http://127.0.0.1:$PORT/"
echo "Door    http://127.0.0.1:$PORT/showing.html   (?state=requested | ?state=approved)"
