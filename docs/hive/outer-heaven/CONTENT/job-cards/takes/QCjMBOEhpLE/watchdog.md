...
**Walk:** 2026-08-14 re-walk · Nate 82 shortlist · video · 14:32 · 3372 words

## A. Source Map

Speaker/channel: **Nate Herk | AI Automation**. Title: *n8n's NEW Native Data Tables Just Made Building Agents So Much Easier*. Kind: video. Duration: 14:32. Caption words: 3372. Corpus tag: Nate 82 shortlist.
...
1. All right.
2. I'll call this one demo with a backslash or maybe a forward slash.
3. So now that we've done that, if I go back into my end data table for contacts and I hit refresh, we should see those contact rows have come into our instance.
4. So let's go on to a different example here.
5. How many wireless headphones have we sold?
6. For the date, I told it the format that it needs to structure its dates as.
7. So that was interesting.
8. Thanks everyone.
Close: And as always, I appreciate you guys making it to the end of the video. (`QCjMBOEhpLE` @ 14:24)

## B. Atomic Knowledge

### Opening claim
- **Claim:** So, we've got our sales data AI agent right here and it's hooked up to all of these tools so that it can query through our sales database.
...
- **Evidence:** SOURCE quote: “So, we've got our sales data AI agent right here and it's hooked up to all of these tools so that it can query through our sales database.”
...
- **Source:** `QCjMBOEhpLE` @ 0:01
- **Epistemic:** SOURCE
### Named method
- **Claim:** And we're going to first of all get this into our NAND data table.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “And we're going to first of all get this into our NAND data table.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
...
- **Source:** `QCjMBOEhpLE` @ 6:43
- **Epistemic:** SOURCE
### Demo / proof move
- **Claim:** So, I'm going to open up the chat and I'm going to ask it how much total revenue have we made from our Bluetooth speaker.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “So, I'm going to open up the chat and I'm going to ask it how much total revenue have we made from our Bluetooth speaker.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `QCjMBOEhpLE` @ 0:03
- **Epistemic:** SOURCE
### Pricing or scale claim
- **Claim:** Total revenue from Bluetooth speaker was $546.
- **Reasoning:** Speaker used a number to prove value or close a CTA.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “Total revenue from Bluetooth speaker was $546.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Label UNVERIFIED. Steal the *structure* (didn’t-send, napkin, package), not the number.
- **Confidence:** low as FACT
- **Source:** `QCjMBOEhpLE` @ 0:07
- **Epistemic:** SOURCE
### Warning / hedge
- **Claim:** And so you may have never seen this node before.
- **Reasoning:** Speaker named a limit, hedge, or thing the agent should not do.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “And so you may have never seen this node before.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Keep the hedge as the desk. It is usually the steal.
- **Confidence:** high as SOURCE hedge; still not a ship
- **Source:** `QCjMBOEhpLE` @ 0:36
- **Epistemic:** SOURCE
### Close / CTA
...
- And then it just used its calculator tool to figure out that total. — `QCjMBOEhpLE` @ 0:17
- It calculated this by retrieving all sales rows for Bluetooth speaker and then it summed each row's total revenue and that's why it used its calculator and got to the answer 546 which is correct. — `QCjMBOEhpLE` @ 0:27
- And so you may have never seen this node before. — `QCjMBOEhpLE` @ 0:36
- As always, you guys will get this workflow as well as the two example data sets for completely free so that you can get in here and get your hands dirty and just start playing around with it to understand how it works. — `QCjMBOEhpLE` @ 1:06
- So if you don't see this right now, all you're going to have to do is make sure you update your Nitn to the latest version. — `QCjMBOEhpLE` @ 1:23
...
- **Situation:** All right.
- **Action:** I'll call this one demo with a backslash or maybe a forward slash.
- **Reasoning:** Speaker treats the on-screen path as sufficient teaching.
- **Outcome:** So now that we've done that, if I go back into my end data table for contacts and I hit refresh, we should see those contact rows have come into our instance.
- **Lesson:** And so you may have never seen this node before.
- **Implicit rule:** If the demo cannot be re-opened as a file/URL/log, it is theater.
- **Source:** `QCjMBOEhpLE` @ 0:00 · Epistemic: SOURCE (walkthrough) + INFERENCE (lesson)

## F. Decision Rules

- Speaker hedge to keep: And so you may have never seen this node before.
...
- **Tape $ / counts UNVERIFIED:** $546., 10 hours. Speaker said or implied them; I do not promote them to FACT, price analog, or Path A MUST.
...
### Machine: text-to-workflow is a draft
...
- **Procedure:** Generated graph is preview. Click nodes; do not ship the export. Run D. Fail-closed before send/pay/deploy/book/publish.
...
- **Operate-never payload:** builder payload may be non-empty (vendor / dial / post / jackpot / School). Machine still stolen.
...
- This is a native-table / $546 tape. I steal **calculator + open the table**, not 11ms.
- **FLAG: “correct” without me opening the rows.** $546 is his fixture — I still re-sum (`kOKavHnlPik`). `golden-test-loop`. 11ms / 1600ms — **UNVERIFIED** as a product claim.
- **In-instance data is a blast-radius win** (no extra API) and a leak if the table holds PII. Sanitize (`oWdJMJp2HgM`).
...
- **Challenge job (§8):** I keep the disagreements in H. I still steal **text-to-workflow is a draft**.
