...
**Walk:** 2026-08-14 re-walk · Nate 82 shortlist · short · 1:54 · 524 words

## A. Source Map

Speaker/channel: **Nate Herk | AI Automation**. Title: *Gemini's New File Search Just Leveled Up RAG Agents (10x Cheaper)  #artificialintelligence #aiagent*. Kind: short. Duration: 1:54. Caption words: 524. Corpus tag: Nate 82 shortlist.
...
1. Here's how to use Gemini's new file search API in N&.
2. So the third HTTP request that we have to make is we're going to take this file and we're going to move it into the folder that we created earlier in step one.
3. So I'm going to ask the agent what happens if your club breaks during the round.
4. If you want to watch the full breakdown, then click on that play button right here, and I'll see you guys over

## B. Atomic Knowledge

### Opening claim
- **Claim:** Here's how to use Gemini's new file search API in N&.
...
- **Evidence:** SOURCE quote: “Here's how to use Gemini's new file search API in N&.”
...
- **Source:** `KVFfApQZhE4` @ 0:01
- **Epistemic:** SOURCE
### Named method
- **Claim:** So the third HTTP request that we have to make is we're going to take this file and we're going to move it into the folder that we created earlier in step one.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “So the third HTTP request that we have to make is we're going to take this file and we're going to move it into the folder that we created earlier in step one.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `KVFfApQZhE4` @ 0:40
- **Epistemic:** SOURCE
### Demo / proof move
- **Claim:** So today I'm going to be talking about how we can use it in NIDN for our rag agents without having to build a huge massive data pipeline that looks like this because we're able to just drop in a file and get answers back.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “So today I'm going to be talking about how we can use it in NIDN for our rag agents without having to build a huge massive data pipeline that looks like this because we're able to just drop in a file and get answers back.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `KVFfApQZhE4` @ 0:08
- **Epistemic:** SOURCE
### Close / CTA
- **Claim:** If you want to watch the full breakdown, then click on that play button right here, and I'll see you guys over
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “If you want to watch the full breakdown, then click on that play button right here, and I'll see you guys over”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `KVFfApQZhE4` @ 1:47
- **Epistemic:** SOURCE
### Tape claim
- **Claim:** So, we're going to hook up an AI agent to the tool and then we'll be able to chat with it to get all of the information that is in this file which will be in this folder that we created.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “So, we're going to hook up an AI agent to the tool and then we'll be able to chat with it to get all of the information that is in this file which will be in this folder that we created.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `KVFfApQZhE4` @ 0:50
- **Epistemic:** SOURCE
...
- It generates those embeddings and then you're right away able to just chat with it and ask it questions and get answers back. — `KVFfApQZhE4` @ 0:04
- The first one is going to be us actually creating that store. — `KVFfApQZhE4` @ 0:26
- So the third HTTP request that we have to make is we're going to take this file and we're going to move it into the folder that we created earlier in step one. — `KVFfApQZhE4` @ 0:40
- And then finally, we have to set up the actual request to query it. — `KVFfApQZhE4` @ 0:47
- So, we're going to hook up an AI agent to the tool and then we'll be able to chat with it to get all of the information that is in this file which will be in this folder that we created. — `KVFfApQZhE4` @ 0:50
...
- **Situation:** Here's how to use Gemini's new file search API in N&.
- **Action:** So the third HTTP request that we have to make is we're going to take this file and we're going to move it into the folder that we created earlier in step one.
- **Reasoning:** Speaker treats the on-screen path as sufficient teaching.
- **Outcome:** So I'm going to ask the agent what happens if your club breaks during the round.
- **Lesson:** It generates those embeddings and then you're right away able to just chat with it and ask it questions and get answers back.
- **Implicit rule:** If the demo cannot be re-opened as a file/URL/log, it is theater.
- **Source:** `KVFfApQZhE4` @ 0:01 · Epistemic: SOURCE (walkthrough) + INFERENCE (lesson)
...
- Does the pair tape (irg-2IfAjpo) contradict a hedge on this cut?
...
- Paired / overlapping titles on disk: `irg-2IfAjpo`. Walk both; do not ship from the short.
...
### Machine: fixture vs last-known-good retrieval
...
- **Procedure:** Open the cited chunk. Pretty retrieve is not a receipt. Run D. Fail-closed before send/pay/deploy/book/publish.
...
- **Operate-never payload:** rag payload may be non-empty (vendor / dial / post / jackpot / School). Machine still stolen.
...
- Teaser. I steal **source cited**, not 10×.
- **FLAG: one cited answer as a smoke.** Long tape’s total-count fail is missing here. I do not ship from the short.
- **10× cheaper — UNVERIFIED.**
...
- **Challenge job (§8):** I keep the disagreements in H. I still steal **fixture vs last-known-good retrieval**.
