...
**Walk:** 2026-08-14 re-walk · Nate 82 shortlist · short · 3:00 · 812 words

## A. Source Map

Speaker/channel: **Nate Herk | AI Automation**. Title: *How I INSTANTLY Generate Proposal Decks with n8n AI Agents*. Kind: short. Duration: 3:00. Caption words: 812. Corpus tag: Nate 82 shortlist.
...
1. Here's how I generate proposal decks for my clients automatically.
2. So if that sounds like a use case you might be interested in understanding, then let's go ahead and jump into it and I'm going to start off up here with this first workflow.
3. So, this is why we pin this data here as if we just basically added this row and now we have to process it.
4. If you want to watch the full breakdown, then click on that play button right here.

## B. Atomic Knowledge

### Opening claim
- **Claim:** Here's how I generate proposal decks for my clients automatically.
...
- **Evidence:** SOURCE quote: “Here's how I generate proposal decks for my clients automatically.”
...
- **Source:** `-Q_P7HFydZk` @ 0:00
- **Epistemic:** SOURCE
### Named method
- **Claim:** So, that's just a quick guardrail right there.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “So, that's just a quick guardrail right there.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `-Q_P7HFydZk` @ 2:18
- **Epistemic:** SOURCE
### Demo / proof move
- **Claim:** So, in today's video, I'm going to be going over this workflow, and we're going to pretend you're in the scenario where you're hopping off a call with a potential client, and now you want to follow up with a slide deck proposal.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “So, in today's video, I'm going to be going over this workflow, and we're going to pretend you're in the scenario where you're hopping off a call with a potential client, and now you want to follow up with a slide deck proposal.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
...
- **Epistemic:** SOURCE
### Warning / hedge
- **Claim:** We will get human approval right here to see if we want to have a slide deck generated or not, cuz we don't always need that.
- **Reasoning:** Speaker named a limit, hedge, or thing the agent should not do.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “We will get human approval right here to see if we want to have a slide deck generated or not, cuz we don't always need that.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Keep the hedge as the desk. It is usually the steal.
- **Confidence:** high as SOURCE hedge; still not a ship
- **Source:** `-Q_P7HFydZk` @ 0:45
- **Epistemic:** SOURCE
### Close / CTA
- **Claim:** If you want to watch the full breakdown, then click on that play button right here.
- **Reasoning:** Speaker offered this as how the system works or why a viewer should believe the demo.
- **Mechanism:** Talk + on-screen walkthrough. Not an independent receipt.
- **Evidence:** SOURCE quote: “If you want to watch the full breakdown, then click on that play button right here.”
- **Conditions:** If the viewer treats the captioned demo as the whole method.
- **Exceptions:** If the long-form pair, a fail, or a later hedge contradicts the teaser.
- **Action:** Store the claim. Do not operate the payload. Challenge in H.
- **Confidence:** medium as SOURCE-of-speech; low as business fact
- **Source:** `-Q_P7HFydZk` @ 2:53
- **Epistemic:** SOURCE
...
- The first part up here is just logging the meeting once the meeting actually ends. — `-Q_P7HFydZk` @ 0:24
- And it logs it in a Google sheet which we'll be looking at which is going to look like this that has the date, the title, the attendees, the gist of the meeting, the ID, and then a status. — `-Q_P7HFydZk` @ 0:29
- And then the second part of the workflow is the actual slide deck creation. — `-Q_P7HFydZk` @ 0:35
- We will get human approval right here to see if we want to have a slide deck generated or not, cuz we don't always need that. — `-Q_P7HFydZk` @ 0:45
- And if we do, then we'll go ahead and pass it over to the proposal generator agent, which will shoot off an API call to Gamma and deliver us a ready to go slide deck that is super professional. — `-Q_P7HFydZk` @ 0:52
...
- **Situation:** Here's how I generate proposal decks for my clients automatically.
- **Action:** So if that sounds like a use case you might be interested in understanding, then let's go ahead and jump into it and I'm going to start off up here with this first workflow.
- **Reasoning:** Speaker treats the on-screen path as sufficient teaching.
- **Outcome:** So, this is why we pin this data here as if we just basically added this row and now we have to process it.
- **Lesson:** We will get human approval right here to see if we want to have a slide deck generated or not, cuz we don't always need that.
- **Implicit rule:** If the demo cannot be re-opened as a file/URL/log, it is theater.
- **Source:** `-Q_P7HFydZk` @ 0:00 · Epistemic: SOURCE (walkthrough) + INFERENCE (lesson)

## F. Decision Rules

- Speaker hedge to keep: We will get human approval right here to see if we want to have a slide deck generated or not, cuz we don't always need that.
...
- Does the pair tape (KGXFkUlBHxw) contradict a hedge on this cut?
...
- Paired / overlapping titles on disk: `KGXFkUlBHxw`. Walk both; do not ship from the short.
...
### Machine: outline exists, no send
...
- **Procedure:** Deck is a draft. Client send is the hard step. Run D. Fail-closed before send/pay/deploy/book/publish.
...
- **Operate-never payload:** proposal payload may be non-empty (vendor / dial / post / jackpot / School). Machine still stolen.
...
- Teaser. I steal the **approval + last-item guardrail**, not “instantly.”
- **FLAG: instantly = skipped verify.** Title says INSTANTLY; the long tape still has wait/poll, approval, and “always a little random.” I do not ship from the short.
- **Last-item limit is a Watchdog win.** Collision path is a named fail. `golden-test-loop`.
...
- **Challenge job (§8):** I keep the disagreements in H. I still steal **outline exists, no send**.
