...
interface WorkflowJobPayload {
  ...
  userId: string;
  service: 'ai-saas' | 'research' | 'content' | 'support' | 'analytics';
  subService?: string;
...
// Send webhook to n8n
async function sendN8nWebhook(endpoint: string, data: any, workflowRunId: string): Promise<void> {
...
async function processWorkflow(job: Job<WorkflowJobPayload>): Promise<any> {
...
async function processAISaaSWorkflow(data: WorkflowJobPayload): Promise<any> {
...
async function processResearchWorkflow(data: WorkflowJobPayload): Promise<any> {
...
async function processContentWorkflow(data: WorkflowJobPayload): Promise<any> {
...
async function processSupportWorkflow(data: WorkflowJobPayload): Promise<any> {
...
async function processAnalyticsWorkflow(data: WorkflowJobPayload): Promise<any> {
  const { workflowRunId, aiConfig } = data;
...
async function triggerAuxiliaryWorkflows(workflowRunId: string, service: string, result: any): Promise<void> {
  try {
