...
const raw = {
  meta: {
    ...
    thesis:
      'Friday cash is in. The book is still the same seven U.S. mega-cap growth lines. A mixed close into a weekend is not an add ticket.',
    thesisLead: 'Friday cash is in. Same book.',
    thesisAccent: 'A mixed close is not an add.',
    catalyst:
      'U.S. 10-year climbed to 5% on the AP Friday tape. Chips still bid. Next cash is Monday, not a new holding.',
    kicker:
      'Saturday wake. US, CA, Europe, and Tokyo are closed. Last cash is Friday 2026-09-18.',
  ...
  market: {
    ...
    note: 'Friday cash: S&P 7,650.50 +0.17%. Nasdaq 26,522.55 +0.40%. AP: 10-year climbed to 5.00%. NVDA closed $222.27 +1.34%.',
    nextCalendar: {
      label: 'Monday cash open',
      detail: 'Weekend. Mac mini / Studio stores Tue Sep 22. Do not treat a closed tape as a sourced add.',
    },
  },
  markets: {
    global: {
      indices: [
        {label: 'Nikkei 225', value: nikkei.toLocaleString('en-US'), dayPct: nikkeiDayPct, note: 'Friday Tokyo (Nikkei / Zaikei). Third up day. Reclaimed 65,000.'},
        {label: 'TOPIX', value: topix.toLocaleString('en-US'), dayPct: topixDayPct, note: 'Friday Tokyo (Nikkei / Zaikei). Broad tape down while Nikkei rose.'},
        {label: 'Hang Seng', value: hangSeng.toLocaleString('en-US'), dayPct: hangSengDayPct, note: 'Friday close (National Times / Armenpress)'},
        {label: 'Shanghai Composite', value: shanghai.toLocaleString('en-US'), dayPct: shanghaiDayPct, note: 'Friday close (National Times / Armenpress)'},
        {label: 'FTSE 100', value: ftse.toLocaleString('en-US'), dayPct: ftseDayPct, note: 'Friday London (AJ Bell / MarketScreener)'},
        {label: 'DAX 40', value: dax.toLocaleString('en-US'), dayPct: daxDayPct, note: 'Friday Frankfurt (Stooq)'},
        {label: 'CAC 40', value: cac.toLocaleString('en-US'), dayPct: cacDayPct, note: 'Friday Paris (Stooq / AFP via France 24)'},
      ],
      fx: [{label: 'USD/JPY', value: '157.45', note: 'Around 0810 GMT Friday (National Times). No Saturday official reprint.'}],
      commodities: [
        {label: 'WTI Oct', value: `$${wti.toFixed(2)}`, dayPct: wtiDayPct, note: 'Friday settle −$1.61 (EnergyNow / DTN / MarketScreener)'},
        {label: 'Brent Nov', value: `$${brent.toFixed(2)}`, dayPct: brentDayPct, note: 'Friday settle −$0.95 (EnergyNow / DTN / FinancialJuice)'},
      ],
      rates: [
        {label: 'U.S. 10-year', value: `${tenYear.toFixed(2)}%`, note: 'AP Friday session: climbed to 5.00%. Official Treasury par row for 09/18 unread at this wake. Last official FRED DGS10 is 4.94% on 09/17.'},
      ...
      note: 'Saturday. Asia, Europe, and U.S. cash are Friday closes.',
    },
    us: {
      indices: [
        {
      ...
      sectors: [
        ...
        {label: 'Energy', note: 'Oil settled lower. Sector level unread — omitted.'},
      ],
      breadth:
        'Friday: NYSE decliners led ~1.78-to-1. Nasdaq 1,973 up / 2,810 down (1.42-to-1). S&P 5 new highs / 30 new lows. Nasdaq 45 new highs / 162 new lows (Reuters).',
      yields: [
        {
          label: 'U.S. 10-year',
          value: `${tenYear.toFixed(2)}%`,
          note: 'AP Friday: climbed to 5.00%. Treasury par table 09/18 unread. FRED DGS10 last official 4.94% on 09/17.',
        },
      ],
      note: 'Friday cash. AP YTD: S&P +11.8%. Week: S&P −0.1%, Nasdaq +0.7%, Dow −1.7% (AP).',
    },
    ca: {
      indices: [
        {
      ...
      cadUsd: '1.4002 CAD per USD (BoC Valet daily average 2026-09-18).',
      note: 'Friday cash. Saturday TSX closed. BoC overnight still 2.25% from the last hold. Next decision Oct 28 (Reuters, prior tape).',
    },
    calendar: {
      items: [
        {
          ...
          why: 'US, CA, Europe, and Tokyo are shut. Friday is the last print. Do not invent a Saturday price.',
        },
        {
          when: 'Monday Sep 21',
          where: 'US' as const,
          label: 'U.S. / CA cash open',
          why: 'First session after the mixed Friday close and a 5% 10-year tape. Do not add into the open.',
        },
        {
          when: 'Tuesday Sep 22',
          where: 'US' as const,
        ...
        },
        {
          ...
          why: 'Already voted 7–2 around 1.25%. Effective date, not a new vote.',
        },
        {
          when: 'Oct 1',
          where: 'US' as const,
  ...
  holdings: [
    ...
    },
    {
      ...
      whatMatters: 'Friday $336.13 −0.26%. iPhone 18 Pro was in stores Friday. Do not add the same name this weekend.',
    },
    {
      ticker: 'VOO',
      ...
      whatMatters: 'Friday $701.78 +0.11%. New core money still simplifies here — after Monday cash, not on a Saturday.',
    },
    {
      ticker: 'VTI',
      rating: 'HOLD',
    ...
    },
    {
      ...
      tone: 'long' as const,
      role: 'Income sleeve',
      whatMatters: 'Friday market $28.95 −0.41% (MarketWatch). Different job. NAV unread this sitting.',
    },
  ],
  portfolio: {
    concentrationThesis: 'VOO + VTI + VUG + MGK + AAPL + NVDA still buy the same companies.',
    concentrationBody:
      'Friday’s chip bid and Europe’s red close move several Wealthsimple lines together. That is structure, not a sell ticket. A weekend mark is not new capital.',
  ...
  names: [
    {
      ...
      streakNote: 'Thursday close $219.34 (MarketBeat prior). Friday $222.27 +1.34% (Stooq / MarketBeat). One cell only.',
      fundamentals: [
      ...
      interpretation: {
        chips: [
          ...
          {label: 'CONFIRMED', tone: 'watch' as const, text: 'AP Friday: 10-year at 5.00%. SOX +2.78%. NVDA $222.27.'},
          {
            ...
            text: 'Buying Friday’s bid into a closed weekend is a timing bet we do not have to take.',
          },
        ],
        note: 'No composite score. One Friday day-change is a count, not a 73/100.',
      },
      actionMatrix: {
        headline: 'HOLD the line. Do not add on a Saturday.',
        rows: [
          {
            tone: 'long' as const,
    ...
    },
    {
      ...
      price: aaplClose,
      dayPct: aaplDayPct,
      holdNote: 'Friday close $336.13. You just bought it. Launch day is over. Do not add.',
      catalyst: {
        headline: 'iPhone 18 Pro was in stores Friday. Mac mini / Studio in stores Sep 22.',
        steps: [
          'Phone launch day happened (Canada and U.S.)',
    ...
    },
    {
      ...
      copy: {
        ...
        body: 'Do not sell. Do not split the next contribution with VTI. Do not add on a Saturday.',
      },
    },
    {
      ticker: 'VTI',
  ...
  unknowns: [
    ...
    },
    {
      ...
      whyItMatters: 'AP said the yield climbed to 5.00%. FRED DGS10 last official is 4.94% on 09/17. A session quote is not the par row.',
      neededToKnow: 'Treasury TextView / H.15 row dated 09/18.',
  ...
  capitalPlan: {
    ...
    freshCapital: 'No add on a Saturday. New core money simplifies into VOO after Monday cash.',
    ...
    nextTrigger: 'Monday Sep 21 — U.S. / CA cash. Mac mini / Studio Tue Sep 22. BoJ call effective Thu Sep 24.',
    ifThen: [
      {
        tone: 'long' as const,
        if: 'Monday cash holds without a chip crash',
  ...
  risks: [
    ...
    },
    {
      ...
      body: `AP Friday: 10-year climbed to ${tenYear.toFixed(2)}%. Official par row unread. FOMC hiked 12–0. BoJ hiked 7–2. Long rates still compress the exact overweight.`,
    },
    {
      n: '03',
  ...
  close: {
    kicker: 'DIAGNOSTIC',
    headline: 'Friday’s mixed close is a book mark. Saturday is not a buy ticket.',
    body: 'Same seven lines. Same overlap. Sell nothing this weekend. Do not add NVDA because SOX bid into the close. If fresh capital arrives after Monday cash, it still wants VOO first.',
    pills: [
      {tone: 'watch' as const, label: 'HOLD THIS WEEKEND'},
      {tone: 'caution' as const, label: 'NO ADD ON A SATURDAY'},
