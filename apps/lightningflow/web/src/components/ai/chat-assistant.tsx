...
import { UniversalAssistant, Industry, UserRole, BusinessGoal, createAssistant } from '../../lib/ai/universal-assistant';
...
export function ChatAssistant({ 
  ...
  useEffect(() => {
    ...
    const initAssistant = async () => {
      try {
        ...
        // Don't pass apiKey - the assistant will use server-side proxy calls
        ...
        if (isMounted && typeof toast?.error === 'function') {
  ...
  }, [industry, role, userId]); // Removed priceUSD and breakdown from dependencies

  // Update BTC context when it changes (without re-initializing assistant)
  useEffect(() => {
    if (assistant && priceUSD > 0) {
      assistant.setBTCContext({
        price_usd: priceUSD,
        breakdown
      });
    }
  }, [assistant, priceUSD, breakdown]);
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-4 pr-2">
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] min-w-[120px] rounded-lg p-3 break-words ${
                message.type === 'user' 
                ...
                <p className="text-sm whitespace-pre-wrap break-words overflow-wrap-anywhere word-break-break-all">{message.content}</p>
                ...
                {message.btc_info && (
                  <div className="mt-2 p-2 bg-orange-50 dark:bg-orange-950 rounded border-l-2 border-orange-500">
                    <div className="flex items-center gap-2 text-xs flex-wrap">
                      ...
                      <span className="whitespace-nowrap">₿ {message.btc_info.amount_btc}</span>
                      <span className="whitespace-nowrap">≈ ${message.btc_info.amount_usd}</span>
                      <span className="whitespace-nowrap">({message.btc_info.amount_sats.toLocaleString()} sats)</span>
                    </div>
                  </div>
                )}
