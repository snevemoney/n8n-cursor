...
export const DataTable = memo(function DataTable({ columns, data }: DataTableProps) {
  ...
  const getCellContent = (content: ReactNode, colKey: string, rowIdx: number): string => {
    ...
    if (isValidElement(content) && typeof content.props.children === 'string') {
      return content.props.children;
    }
    if (isValidElement(content) && typeof content.props.children === 'object' && content.props.children) {
          ...
          {data.map((row, idx) => {
            ...
            return (
              <React.Fragment key={idx}>
                <tr 
                  className="border-b border-white/5/20 hover:bg-white/5 transition-colors duration-100 cursor-pointer focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-400 focus-visible:outline-offset-2"
