...
import { Brain, Code, TrendingUp, Zap, Building2, DollarSign, GraduationCap, FileCheck, PenTool } from 'lucide-react';
  ...
  const loadAgents = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setAgents(data.success ? data.data.agents : data.agents || []);
      }
    } catch (err) {
            ...
            {agents.map((agent) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-lg border cursor-pointer transition-all focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-400 focus-visible:outline-offset-2 ${
                    isSelected
