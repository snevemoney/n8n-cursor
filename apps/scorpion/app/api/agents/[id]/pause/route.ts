...
/**
 * POST /api/agents/[id]/pause - Pause an agent (set to standby)
 */
export const POST = withErrorHandling(async (
  ...
  if (storedAgent) {
    const updated = await updateAgentStatus(id, 'standby');
    if (!updated) {
    ...
    return createSuccessResponse({
      ...
      message: `Agent "${updated.name}" paused successfully`
    });
  }
  ...
  councilMemberStatusCache.set(id, 'standby');
  
  return createSuccessResponse({
    id,
    codename: councilMembers[memberIndex].name,
    status: 'standby',
    message: `Agent "${councilMembers[memberIndex].name}" paused successfully`
  });
});
