...
|---|---|---|---|---|---|---|
| stop | `stop`, `never mind`, `shut up` | `stop` | mouth/turn.py local | unit | pass | UNKNOWN |
| greet | `hello`, `hey`, `hi jarvis` | `greet` | mouth/turn.py local | unit | pass | quoted `hey` → Sir. Hey Evens. What are we working on? |
...
| safari | `go to YouTube`, `open YouTube`, `scroll down` | `safari` | hands/see.py safari_act | unit | pass | quoted `Hi Jarvis open YouTube` → Sir. Safari, as requested. Safari: J.A.R.V.I.S.. http://127.0.0.1:4018/ |
...
| cursor | `check my repo`, `look at the code`, `fix this bug` | `cursor` | brain/online.py call_cursor_turn | unit | pass | UNKNOWN |
| converse | `what's going on`, `tell me a joke`, `can you` | `converse` | brain/online.py call_cursor_turn | unit | pass | UNKNOWN |
...
| pro | `what is marketing`, `brief me on marketing`, `BUS 204` | `pro` | hands/pro.py brief | unit | pass | UNKNOWN |
